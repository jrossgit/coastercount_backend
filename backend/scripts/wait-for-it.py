...
if __name__ == "__main__":
    ...
    try:
        ...
        if os.environ.get("RABBITMQ_DISABLE") != "1":
            LOGGER.info("Checking rabbitmq connection")
            fleet_rabbitmq_management_url_split = os.environ[
                "RABBITMQ_MGMT_URL"].split(":")

            if not len(fleet_rabbitmq_management_url_split) == 2:
                raise Exception("Invalid RabbitMQ URL provided")

            ssl_mode = False
            if os.environ.get("RABBITMQ_MGMT_HTTPS", "") != "":
                ssl_mode = True

            check_connection(
                host=fleet_rabbitmq_management_url_split[0],
                port=fleet_rabbitmq_management_url_split[1],
